...
terraform {
  backend "s3" {
    ...
    key            = "PSA_state_files/cohort/dailybudgets/tfstate"
    region         = "eu-west-2"
...
# template = file("${path.module}/templates/email-sns-stack.json.tpl")
  ...
  vars = {
    display_name = var.BAdisplay_name
    subscriptions = join(
      ",",
      formatlist(
        "{ \"Endpoint\": \"%s\", \"Protocol\": \"%s\"  }",
        var.BAemail_addresses,
        var.protocol,
      ),
    )
  }
...
# template = file("${path.module}/templates/email-sns-stack.json.tpl")
  template = file("email-sns-stack.json.tpl")
  vars = {
    display_name = var.BDdisplay_name
    subscriptions = join(
      ",",
      formatlist(
        "{ \"Endpoint\": \"%s\", \"Protocol\": \"%s\"  }",
        var.BDemail_addresses,
        var.protocol,
      ),
    )
  }
